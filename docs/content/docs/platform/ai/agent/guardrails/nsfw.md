...
Like Jailbreak, NSFW reads the `Model` child attached to the parent `Check For Violations`. Without one, the cluster element throws a configuration error and forces fail-closed regardless of **Fail Mode**.
