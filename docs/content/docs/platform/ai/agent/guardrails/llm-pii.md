...
Reads the `Model` child attached to the parent `Check For Violations` (or `Sanitize Text`). Without one, the cluster element throws a configuration error and forces fail-closed regardless of **Fail Mode**.
...
Both run in the LLM stage, so they consume preflight-masked text. The classifier sees, e.g., `"<EMAIL> lives at 123 Main St"` rather than the raw email — but the address (which the rule-based PII detector doesn't know how to find) is still in the visible text for the classifier to flag.
...
2. **All-hallucinated fail-closed**: if every returned span fails substring verification, the check escalates to a "100% hallucination" exception and the advisor treats it as a check failure → fail-closed by default.
