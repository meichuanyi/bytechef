...
| **Fail Mode** | What to do when the check itself cannot run. Invalid regex is a configuration error and forces fail-closed regardless |
...
A bad pattern is treated as a **configuration error** and forces fail-closed even if you picked Fail Open — a broken regex is an operator bug, not a transient outage, and silently allowing requests through a dead detector is worse than blocking until someone fixes it.
