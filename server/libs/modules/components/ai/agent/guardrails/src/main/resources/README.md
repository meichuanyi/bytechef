...
| `Keywords` | LLM | Word-boundary-aware match against a configured keyword list | block / mask |
...
```
   ...
   inbound text →  │ CheckForViolations │
        ...
        (PII, SecretKeys,            (Keywords, Jailbreak,
         URLs, CustomRegex)           NSFW, TopicalAlignment,
              │                       Custom, LlmPii)
              ▼                           │
         masked text  ────────────────────▶
              │                           │
...
Masking rule-based checks (PII, Secret Keys, URLs, Custom Regex) implement the
`PreflightMasking` mixin. When they detect content they emit `MaskResult.Entities(...)` describing
the spans to redact; the advisor then merges every preflight check's entities and applies them
**longest-first** so overlapping matches (an email contains a domain; a JWT contains base64
fragments) don't leave half-masked fragments.
...
(`<EMAIL>`, `<AWS_ACCESS_KEY>`, `<URL>` placeholders) to reason about prompt-injection or
on-topic-ness signals.

Keyword matching runs in the LLM stage on purpose: a keyword list authored against raw input
breaks as soon as PII / URL detection masks part of the matched span. By running keywords against
the masked text the operator can write `["AKIA"]` without worrying about whether the secret-key
detector ate the prefix.

## Fail mode

Every child carries a `Fail Mode` property with two options:

- **Fail closed (block on check failure)** — if the check itself cannot run (LLM outage, missing
  `MODEL` child, runtime exception), the request is blocked. Default. Use this when content
  policy is more important than availability.
- **Fail open (allow on check failure)** — if the check itself cannot run, the request is forwarded
  as if the check had passed and the failure is recorded to telemetry under
  `guardrail.skippedFailures`. Use this when uptime matters more than full coverage.

**Fail mode does not affect successful checks.** A check that runs and returns a violation
**always** blocks the request, regardless of mode. Fail mode only governs what happens when the
check infrastructure itself fails.

**Configuration errors override fail mode.** If a user supplies an invalid regex, an unparseable
threshold, or otherwise misconfigures the check, the advisor's `isConfigurationError` branch
forces the request closed even when the user picked Fail Open. Operator bugs are not the same as
transient outages, and silently allowing requests through a guardrail that's been broken for
weeks is worse than blocking until someone fixes it.

## Telemetry

Two metadata keys are attached to the response so downstream observability picks up guardrail
...
| `guardrail.skippedFailures` | Any response (blocked or unblocked) | List of guardrail names + exception kinds for checks that ran under FAIL_OPEN and crashed |

Subscribe to `guardrail.skippedFailures` to alert on "guardrail silently degraded" — a guardrail
that consistently fails open without anyone noticing is the same as not having a guardrail at all.
...
The `LlmClassifier` utility additionally fences user input with random nonce tags so the model
can syntactically distinguish operator instructions from user content. The default prompts and
the nonce fencing are independent layers; both run.

## Authorship

The detector library (`PiiDetector`, `SecretKeyDetector`, `UrlDetector`, `KeywordMatcher`,
`RegexParser`, `MaskEntityMap`, `LlmPiiDetector`, `LlmClassifier`) is shared between cluster
elements. `RegexParser.bounded(...)` enforces a per-match character budget so a pathological user
regex can't hang the matcher; the budget is also hit by JDK's catastrophic-backtracking patterns
on long inputs, and the resulting `RegexExecutionLimitException` is treated as a configuration
error (always fail-closed).
