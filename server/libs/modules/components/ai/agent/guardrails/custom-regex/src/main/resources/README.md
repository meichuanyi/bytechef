...
| `failMode` | no | `FAIL_CLOSED` | What to do when the check itself cannot run. Invalid regex is a configuration error and forces FAIL_CLOSED regardless |

The single-form `name` + `regex` properties were removed in PR #4915 review — only the `patterns`
array is supported. Add one entry per pattern; entries run in declaration order and all matches
are aggregated.
...
Both forms run through `RegexParser.compile`, which enforces:
...
A bad pattern is treated as a **configuration error** and forces fail-closed even if the user
picked Fail Open — a broken regex is an operator bug, not a transient outage, and silently
allowing requests through a dead detector is worse than blocking until someone fixes it.
