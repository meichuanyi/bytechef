...
- **`llmPiiCheck`** — emits a `Violation.PatternViolation` listing the entity types the
  classifier reported.
...
`"<EMAIL> lives at 123 Main St"` rather than the raw email — but the address (which the
...
123-45-6789" when no such substring exists. `LlmPiiDetector` defends against this in two
layers:
...
2. **All-hallucinated fail-closed**: if every returned span fails substring verification, the
   detector throws `GuardrailUnavailableException` with a "100% hallucination" message. The
   advisor treats this as a check failure → fail-closed by default.
