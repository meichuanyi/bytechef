...
| `failMode` | no | `FAIL_CLOSED` | What to do when the check itself cannot run (LLM outage, missing `MODEL` child) |
...
the advisor treats as a configuration error → fail-closed regardless of `failMode`.
