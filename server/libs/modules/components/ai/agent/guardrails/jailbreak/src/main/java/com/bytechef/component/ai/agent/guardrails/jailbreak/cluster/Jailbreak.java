...
public final class Jailbreak {
    ...
    private Optional<Violation> apply(String text, GuardrailContext context) throws Exception {
        // Fail-closed via exception: CheckForViolationsAdvisor converts this to a blocking violation, matching the
        // advisor's documented fail-closed policy. Silently returning empty would disable the guardrail without
        // operator feedback.
        ChatClient chatClient = context.chatClient()
            .orElseThrow(() -> new MissingModelChildException("jailbreak"));
    ...
    static Optional<Violation> classifyWith(
        ...
        Verdict verdict = LlmClassifier.classify("jailbreak", chatClient, systemMessage, userPrompt, text, threshold);
